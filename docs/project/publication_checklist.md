...
| Rutherford production executor | `production_histories.png`, `current_sheet_aspect_ratio.png`, optional fixed-scale GIFs | `validation` for partial chunks; `production` only after passing promotion | Ready as restart/resume execution evidence; the `adcc714` GPU lane has one passing production-promotion response bundle. |
...
| Rutherford production physics | long-run histories, convergence figures, seed-QI evidence, fixed-scale movies, response diagnostics, and promotion report | `production` for positive-response current-sheet campaigns after full target completion and a passing `rutherford-promotion-check` report | The `20260525` GPU campaign passes the response-promotion gate; remaining paper-grade work is Rutherford algebraic scaling across amplitudes and equilibria. |
