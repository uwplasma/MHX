...
| --- | --- | --- |
| ![Double-Harris reconnection replay](../_static/readme/double_harris_reconnection.gif) | Many-plasmoid double-Harris chain from a `1024×1024`, `t_end=60` reduced-MHD run: magnetic flux $\psi$ with total flux contours over a 25.1×25.1 periodic double-Harris domain. | Exploratory plasmoid-formation demonstration generated by `examples/media/double_harris_reconnection.py` from a broadband-seeded `examples/plasmoid_chain/plasmoids.py` run; not a converged Sweet-Parker or plasmoid-scaling production claim, and X/O tracking is deferred. |
| ![Double-Harris current sheet](../_static/readme/double_harris_current_sheet.gif) | Full-domain many-plasmoid chain from the same run, showing out-of-plane current density $j_z$ with total flux contours across both sheets. | Exploratory plasmoid-formation demonstration generated by `examples/media/double_harris_reconnection.py` from the same plasmoid-example trajectory; useful for morphology QA, not converged plasmoid-statistics evidence. |
...
| ![Seeded double-Harris flux](../_static/validation/periodic_double_harris_seeded_long_run/figures/periodic_double_harris_flux.gif) | Historical committed validation replay at `64×64`, showing magnetic-flux evolution over `t_end=30`; README landing-page media now uses the separate 1024×1024 many-plasmoid demonstration documented below. | Validation bridge from Harris tearing to longer nonlinear campaigns; bounded evidence, not converged Rutherford/plasmoid production. |
...
The README Harris pair now displays a **many-plasmoid double-Harris chain** from a
high-resolution reduced-MHD run. The underlying field is the spectral analogue of a
pair of thin Harris sheets separated by `sheet_sep`:
...
seeded with broadband white noise so *many* tearing modes grow together into a
plasmoid chain rather than a single seeded island:

$$
\delta\psi(x,y)=\epsilon\,N(x,y),
$$

where $N(x,y)$ is drawn from a white-noise random field. The movies are generated
by `examples/plasmoid_chain/doc_movies.py` from the saved high-resolution trajectory
(1024×1024 on a 25.1×25.1 periodic domain, `t_end=60`):

- the reconnection replay shows magnetic flux $\psi$ with total flux contours;
- the current-sheet companion shows out-of-plane current density $j_z$ with the
  same total-flux contour context.

This is an exploratory demonstration of plasmoid formation, not a converged
Sweet-Parker or plasmoid-scaling production claim; X/O marker tracking is
deferred. The committed GIFs ship in the repository. CI checks their hashes, links,
types, and size budgets without re-running the heavy plasmoid simulation.

![Many-plasmoid double-Harris chain: magnetic flux with flux contours](../_static/readme/double_harris_reconnection.gif)

![Many-plasmoid double-Harris chain: current density with flux contours](../_static/readme/double_harris_current_sheet.gif)
...
- [Schematic generator](https://github.com/uwplasma/MHX/blob/main/examples/media/supplemental.py)
...
## Safe media regeneration

Run or inspect the explicit source campaigns, then render every available case into the ignored staging directory:
...
Inspect the staged files before deliberately copying selected assets into
`docs/_static`. After updating the corresponding manifest hashes, validate the
committed result without regenerating it:
